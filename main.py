...
class SubscriptionApp:
    ...
    def log_notification(self, type_char, text_content, amount_val=0, img_url="", is_bank=False):
    ...
    def show_finance_dialog(self, is_withdraw: bool):
        ...
        img_type_dropdown = ft.Dropdown(
            ...
            value="bank",
            visible=not is_withdraw
        )
        img_in = ft.TextField(label="رابط الصورة (اختياري)", visible=not is_withdraw)
        ...
        def confirm_transaction(e):
            ...
            if is_withdraw:
                ...
                t_char = "d"
                img_val = img_in.value.strip()
                bank_val = (img_type_dropdown.value == "bank")

            self.page.dialog.open = False
            self.show_loading(True)
            self.log_notification(type_char=t_char, text_content=msg, amount_val=amt, img_url=img_val, is_bank=bank_val)
            self.load_data()

        dialog_controls = [amount_in]
        if not is_withdraw:
            dialog_controls.extend([img_type_dropdown, img_in])

        dialog = ft.AlertDialog(
            title=ft.Text(title_text, text_align="right", size=16),
            content=ft.Column(controls=dialog_controls, tight=True, spacing=10),
            actions=[
